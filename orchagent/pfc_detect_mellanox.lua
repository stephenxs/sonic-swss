...
if timestamp_last ~= false then
    ...
    local pfcwd_min_poll_time = redis.call('HGET', 'TIMESTAMP', 'pfcwd_min_poll_time')
    if pfcwd_min_poll_time == false or tonumber(pfcwd_min_poll_time) > real_poll_time then
        redis.call('HSET', 'TIMESTAMP', 'pfcwd_min_poll_time', real_poll_time)
    end
    local pfcwd_max_poll_time = redis.call('HGET', 'TIMESTAMP', 'pfcwd_max_poll_time')
    if pfcwd_max_poll_time == false or tonumber(pfcwd_max_poll_time) < real_poll_time then
        redis.call('HSET', 'TIMESTAMP', 'pfcwd_max_poll_time', real_poll_time)
    end
    if real_poll_time < poll_time * 0.5 then
        -- Skip this occurance since the interval is too short and Write poll_time_adjust back
        redis.call('HSET', 'TIMESTAMP', 'pfcwd_poll_time_adjust', poll_time_adjust_string)
        return rets
    end
    new_adjust = real_poll_time - poll_time
    effective_poll_time = real_poll_time + poll_time_adjust
    if effective_poll_time < poll_time then
        effective_poll_time = poll_time
    else
        ...
        if detection_time then
            ...
            if queue_index and port_id then
                ...
                if occupancy_bytes and packets and pfc_rx_packets and pfc_duration then
                    ...
                    local occupancy_bytes_last = redis.call('HGET', counters_table_name .. ':' .. KEYS[i], 'SAI_QUEUE_STAT_CURR_OCCUPANCY_BYTES' .. '_last')
                    ...
                    if packets_last and pfc_rx_packets_last and pfc_duration_last and occupancy_bytes_last then
                        packets_last = tonumber(packets_last)
                        occupancy_bytes_last = tonumber(occupancy_bytes_last)
                        pfc_rx_packets_last = tonumber(pfc_rx_packets_last)
                        pfc_duration_last = tonumber(pfc_duration_last)
                        local storm_condition = (pfc_duration - pfc_duration_last) > (effective_poll_time * 0.8)

                        -- Check actual condition of queue being in PFC storm
                        if (occupancy_bytes > 0 and occupancy_bytes_last > 0 and packets - packets_last == 0 and pfc_rx_packets - pfc_rx_packets_last > 0) or
                            -- DEBUG CODE START. Uncomment to enable
                            ...
                            if time_left <= poll_time then
                                ...
                                local occupancy_string = '"occupancy","' .. tostring(occupancy_bytes) .. '","occupancy_last","' .. tostring(occupancy_bytes_last) .. '",'
                                ...
                                local storm_condition_string = '"storm_condition","' .. tostring((pfc_duration - pfc_duration_last) / effective_poll_time) .. '",'
                                storm_condition_string = storm_condition_string .. '"pfc_duration","' .. tostring(pfc_duration) .. '","pfc_duration_last","' .. tostring(pfc_duration_last) .. '",'
                                local timestamps = '"timestamp","' .. timestamp_string .. '","timestamp_last","' .. timestamp_last .. '","real_poll_time","' .. real_poll_time .. '","poll_time_adjust","' .. poll_time_adjust .. '"'
                    ...
                    redis.call('HSET', counters_table_name .. ':' .. KEYS[i], 'SAI_QUEUE_STAT_CURR_OCCUPANCY_BYTES' .. '_last', occupancy_bytes)
